...
void test_advanced_combo_long_press(void) {
    printf("\n=== 测试组合按键长按 ===\n");
    ...
    // 验证长按事件
    ASSERT_EVENT_EXISTS(100, BTN_STATE_LONG_PRESS);
    
    // 继续保持长按
    time_simulate_pass(500);
    ASSERT_LONG_PRESS_COUNT(100, 1);
    ...
    printf("组合按键长按测试通过: 组合键支持长按功能\n");
}

void test_advanced_partial_combo_press(void) {
    printf("\n=== 测试部分组合按键 ===\n");
    ...
    printf("部分组合按键测试通过: 不完整组合不触发，单键正常工作\n");
}
...
void test_combo_release_timing(void) {
    printf("\n=== 测试组合键释放时序 ===\n");
    ...
    printf("组合键释放时序测试通过: 不同释放顺序不影响组合识别\n");
}