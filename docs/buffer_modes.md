...
- 无锁设计，适用于单生产者单消费者场景
- 缓冲区满时采用覆写策略（丢弃最旧事件），通过 `get_bits_btn_buffer_overwrite_count()` 可查询覆写次数
- 需要C11编译器支持
...
5. **C11 缓冲区的线程安全约束**：默认 C11 缓冲区基于 SPSC（单生产者单消费者）模型，
   要求 `bits_button_ticks()`（生产者）和 `bits_button_get_key_result()` / `bits_button_peek_key_result()`（消费者）
   不会并发访问同一个缓冲区槽位。在以下场景中请注意：
   - **裸机主循环**：两者在同一循环中顺序调用，天然安全
   - **中断 + 主循环**：`bits_button_ticks()` 在定时器中断中调用时，
     若缓冲区满触发覆写，可能与主循环中的读取产生竞态。
     建议确保缓冲区容量足够（通过 `BITS_BTN_BUFFER_SIZE` 宏调整），避免满溢
   - **RTOS 多线程**：若生产者和消费者在不同线程，建议使用 `BITS_BTN_USE_USER_BUFFER`
     模式，提供带互斥保护的自定义缓冲区实现
   - 无论哪种场景，**回调函数**（`bits_btn_result_cb`）始终会被同步调用，不经过缓冲区，不受此约束影响
