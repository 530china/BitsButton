...
> 🔍 **技术详情**：如需了解模拟器的技术实现细节，请参阅 [模拟器技术实现文档](../simulator/ButtonSimulator.md)。
...
模拟器通过适配层与BitsButton库交互，实现Python界面与C库之间的数据交换。根据运行模式的不同，交互方式有所区别：

### 软件模式数据流向

- 用户界面操作 → 按键状态变化 → 内部模拟处理 → 生成事件 → 返回显示

### 硬件模式数据流向

- 用户界面操作 → 按键状态变化 → 通过动态库 → 传递给BitsButton库 → 生成事件 → 返回显示
...
硬件模式用于验证BitsButton库的功能，通过动态库与底层的BitsButton库交互，具有以下特点：

- **库功能验证**：与实际的BitsButton C库进行交互，验证库的各项功能
- **编译要求**：需要安装适当的编译工具链（如GCC、MinGW或Xcode）来构建动态库
- **真实交互**：模拟器通过动态库调用BitsButton库的函数，更接近实际硬件环境
- **PC验证能力**：让开发者可以在PC上验证库的功能，而无需使用实际的嵌入式开发板
- **适用场景**：适合验证BitsButton库的各种功能、性能测试、功能验证等

### 模式选择建议

- **验证模拟器功能**：使用软件模式
- **验证BitsButton库功能**：使用硬件模式
